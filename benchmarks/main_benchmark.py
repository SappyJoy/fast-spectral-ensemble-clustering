...
def optimize_and_benchmark(dataset_name, algorithm_class, n_trials=50):
    ...
    def objective(trial):
        ...
        params = {}
        # You can define hyperparameter spaces here specific to each algorithm
        if algorithm_class == KMeansClustering:
            ...
            params['n_clusters'] = final_n_clusters
            params['affinity'] = trial.suggest_categorical('affinity', ['euclidean', 'manhattan', 'cosine'])
            ...
            # FSEC-specific hyperparameters are already handled in the benchmark function
        ...
        nmi = benchmark_clustering_algorithm(trial, dataset_name, algorithm_class, params)
    ...
    if algorithm_class == FSECClustering:
        ...
        # Remove individual num_clusters_list_i from params
        for i in range(num_clusters_list_size):
            cluster_key = f'num_clusters_list_{i}'
            best_params.pop(cluster_key, None)

    # Benchmark the final run
    final_metrics = benchmark_clustering_algorithm(trial=None, dataset_name=dataset_name, algorithm_class=algorithm_class, hyperparameter_tuning=False)
    result = {
...
def main():
    ...
    n_trials = 5  # Number of Optuna trials per algorithm per dataset
    results = []
